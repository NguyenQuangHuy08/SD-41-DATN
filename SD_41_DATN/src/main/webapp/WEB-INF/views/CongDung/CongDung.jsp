...
<h1>Form Control</h1>
<form action="/cong-dung/add" method="post">
    <label class="form-label">Ten Cong Dung</label>
...
<table class="table table-bordered">
    ...
    <tbody>
    <tr>
        <c:forEach items="${congDung}" var="fm">
        <td>${fm.id}</td>
        <td>${fm.tenCongDung}</td>
        <td>${fm.ghiChu}</td>
        <td>${fm.ngayTao}</td>
        <td>${fm.ngaySua}</td>
        <td>${fm.trangThai}</td>
        <td>
            <a href="/cong-dung/delete/${fm.id}"><button type="button" class="btn btn-outline-primary">Delete</button></a>
            <a href="/cong-dung/view-update/${fm.id}"><button type="button" class="btn btn-outline-primary">View Update</button></a>
        </td>
    </tr>
    </c:forEach>
    </tr>
